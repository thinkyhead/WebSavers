 ...
 *   - https://codepen.io/yashbhardwaj/pen/QWKKgb
 */
...
(function(d, w) {
  ...
  var FPS = 30;             // FPS
  var F = 200;              // Focal length
  var N = 10;               // Number of trajectories
  var VERTEX_MAX = 10;      // Trail length
  var TRAIL_QUALITY = 4000; // Trail quality
  var mu = 0.5;             // Dependence on the previous anchor point

  var canvas = d.getElementById("stringy");
  var ctx = canvas.getContext("2d");

  function bmRandom(mu, sigma) {
  ...
  function bezier3(t, a, b, c, d, e, dst) {
    t /= e;
    const f1 = (1 - t) * (1 - t) * (1 - t),
          f2 = 3 * t * (1 - t) * (1 - t),
          f3 = 3 * t * t * (1 - t),
          f4 = t * t * t;
    dst.x = a.x * f1 + b.x * f2 + c.x * f3 + d.x * f4;
    dst.y = a.y * f1 + b.y * f2 + c.y * f3 + d.y * f4;
    dst.z = a.z * f1 + b.z * f2 + c.z * f3 + d.z * f4;
  }
